...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function sendToBackend(textData, files) {
    ...
    console.log('[v0] Starting file conversions for:', Object.keys(files));
    for (const [key, file] of Object.entries(files)) {
      try {
        console.log('[v0] Converting file:', key, file.name, file.type);
        const base64 = await fileToBase64(file);
        console.log('[v0] ✓ File converted:', key, 'base64 length:', base64.length);
        convertedFiles[key] = {
          name: file.name,
          type: file.type,
          size: file.size,
        ...
        console.error('[v0] File conversion error:', key, err.message);
    ...
    console.log('[v0] Payload ready. Files included:', Object.keys(convertedFiles).length);
    ...
    let response;
    try {
      console.log('[v0] Sending POST to /api/submit-application');
      console.log('[v0] Payload size:', jsonString.length, 'bytes');
    ...
    try {
      ...
      console.error('[v0] Response parse error:', parseErr.message);
      throw new Error('Invalid server response format');
    }

    if (!response.ok) {
      const errorMsg = data.error || `HTTP ${response.status}`;
      console.error('[v0] Response error:', errorMsg);
